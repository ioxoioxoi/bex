...
class Topup extends BaseController
{
    ...
    public function postList_topups() {
        check_token_login();
        $request = request();
        ...
        if ($postData['id_user']) {
            $builder->where('id_user', $postData['id_user']);
        }
        if ($postData['id_base_payment_method ']) {
            $builder->where('id_base_payment_method ', $postData['id_base_payment_method ']);
        }
    ...
    public function postGet_total_topup_by_pg() {
        check_token_login();
        $request = request();
        $postData = (array)$request->getJSON();
        $db = db_connect();
        $query   = $db->query("select 
        (SELECT COALESCE(sum(amount), 0) as binance_pay FROM topup_users where id_base_payment_method = 1) as binance_pay,
        (SELECT COALESCE(sum(amount), 0) as perfect_money FROM topup_users where id_base_payment_method = 2) as perfect_money,
        (SELECT COALESCE(sum(amount), 0) as duitku FROM topup_users where id_base_payment_method = 3) as duitku,
        (SELECT COALESCE(sum(amount), 0) as qris FROM topup_users where id_base_payment_method = 4) as qris;");
    ...
    public function postCreate_topup() {
        check_token_login();
        $request = request();
        $db = db_connect();
        $postData = (array)$request->getJSON();
        $order_id = substr(md5(rand(1,10000).date('YmdHis')), 0, 6);
        $postData['invoice_number'] = 'INV/'.date('Y').'/'.date('m').'/'.$postData['id_user'].'/'.$order_id;
        $builder = $db->table('topup_users');
